...
public class BookingEventHandler : IBookingEventHandler
{
    ...
    {
        var message = $"User with {email} reserved a workstation on {workplace.Floor} floor in Room {workplace.Room} from {reservation.TimeFrom.ToString("D")} to {reservation.TimeFrom.ToString("D")}";
        await _sender.SendMessageAsync(message);
    }
    ...
    {
        var equipment = await _equipmentForWorkplaceService.GetEquipmentByEquipmentForWorkplaceIdAsync(movement.EquipmentId);
        ...
        sb.Append($"from {previousWorkplace.Floor} floor, {previousWorkplace.Room} room, {previousWorkplace.DeskNumber} desk number ");
        sb.Append($"to {destinationWorkplace.Floor} floor, {destinationWorkplace.Room} room, {destinationWorkplace.DeskNumber} desk number ");
